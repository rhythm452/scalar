...
2. The `useHostedZones` hook reads params from the URL, builds a stable query key `["hosted-zones", {search, type, pageSize, cursor, sortBy, sortOrder}]`, and calls `apiFetch` with `credentials: "include"`.
3. Next.js rewrite proxies `/api/*` to the backend base URL so cookies stay same-origin in production.
...
8. The router returns `{items, next_token, total_estimate}`. The frontend caches by query key, renders the Cloudscape Table, and shows `loading` via the segment `loading.tsx` on first load and inline `Spinner` on refetch.
...
2. On submit, `useCreateRecord` mutation posts JSON with `credentials: "include"`. The submit button shows loading; the form stays mounted.
3. Auth dependency identical to the read path; unauthenticated posts return the AWS-shaped 401 envelope.
...
6. On commit, the router returns `201` with `{change: {id, status: "PENDING", submitted_at}, record}`.
...
|-------|---------|---------|
| `/login` | `(auth)/login/page.tsx` | Mock sign-in form; redirects to `/route53` on success |
